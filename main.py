...
with st.sidebar:
    st.markdown(
        """<img src='https://i.ibb.co/SwN5Mjy/Logo-shield-resized.png'>
    ...
    with st.form("questions"):
        age = st.number_input("Please type your age", min_value=18, max_value=120)
        sex = st.radio("Please select your gender", ["female", "male"], horizontal=True)
        weight = st.number_input(
            "Please type your weight (in kg)", min_value=20, max_value=220, value=60
        )
        height = st.slider(
            "Please select your height (in cm)",
            min_value=0.60,
            max_value=2.30,
            value=1.60,
            step=0.01,
        ...
        children = st.number_input(
            "Please type the number of children you have", min_value=0, value=0
        )
        smoker = st.radio(
            "Please specify if you smoke or not", ["yes", "no"], horizontal=True
        )
        region = st.radio(
...
if submit_btn:
    ...
    st.divider()
    st.markdown(
        "#### With these informations, here are your annual charges for insurance:"
    )
